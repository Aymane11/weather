...
def load_config() -> None:
    ...
    if path.exists("config.ini") == False:
        ...
        try:
            ...
            save = Prompt.ask(
                "Missing data in config file! Create config now?", choices=["Y", "N"]
            )
            save_config() if save == "Y" else print("[bold red]Exiting...[/bold red]")
            exit(1)
            ...
            print(
                "[bold red]Please check your internet connection.\nExiting...[/bold red]"
            )
            exit(1)

        except NoSectionError:
            save = Prompt.ask(
                "Configuration file is missing or corrupted! Create config now?",
                choices=["Y", "N"],
            )
            save_config() if save == "Y" else print("[bold red]Exiting...[/bold red]")
            exit(1)
...
def display(data: dict) -> None:
    ...
    print("---------------------------------------")
    print(
        f"Current weather in: [bold blue]"
