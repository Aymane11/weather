...
def load_config():
    try:
        ...
        while True:
            save = input('Missing data in config file! Create config now [Y/N] ? ').upper()
            if save not in ['Y','N'] :
        ...
        while True:
            save = input('Configuration file is missing or corrupted! Create config now [Y/N] ? ').upper()
            if save not in ['Y','N'] :
                save = input('Wrong input. Create config now [Y/N] ? ').upper()
                continue
            else:
...
def args():
    parser = argparse.ArgumentParser(description = "A weather program fetching data from openweathermap API")
    parser.add_argument('-c', '--config', action = 'store_true', help = 'Create config file (city - units - API key)')
